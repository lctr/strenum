...
///
/// The variants are all fieldless variants, but they each can generate *one* of
/// the aforementioned set of string literals -- in particular, each variant
/// dereferences to an instance of the string literal provided when calling this
/// macro, thus each enum generated by this macro implements
/// `std::ops::Deref<&str>` as well as `AsRef<str>` for cheap reference
/// conversions.
///
...
///
/// This effectively allows for unification of `&str` values with `usize` values
/// via a single `1` byte long interface, as the inherent method `as_usize`
/// returns the `usize` value corresponding to the position of a given variant
/// within this total order.
///
/// There is currently a relatively unstable feature allowing clients to select
/// a variant by *indexing* the associated constant `VARIANTS`, which holds all
/// the defined enum variants in an array in the same order as defined.
///
...
///
/// // *UNSTABLE FEATURE*
/// // In fact, we can use the fixed-size array constant `VARIANTS` to query a
/// // variant by *order of definition*, mimicking the compiler-generated
/// // implementation(s) of `PartialOrd`.
/// // NOTE that the only caveat to this is that the provided index fits within
/// // the bounds of the array containing all defined variants.
...
macro_rules! stringy {
    (
        ...
        impl $name {
            ...
            /// If the variant for this enum was defined with additional data,
            /// then it will be returned in a `Some` variant. Otherwise returns
            /// `None`.
            pub fn $field(&self) -> Option<$fieldty> {
...
mod tests {
    ...
    fn test_is_even() {
        stringy! {
            ...
            Three "3" { 3 }
            Four "4" { 4}
            /// Five is a nice number. Notice that we've arbitrarily decided not
            /// to include any associated data for this variant!
            Five "5"
            Six "6" { 6 }
            Seven "7" { 7 }
            Eight "8" { 8 }
